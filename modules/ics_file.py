            ...
            if active:
                if (
                    ...
                    current_event.append(
                        self.pretty_print(line[line.find(":") + 1 :].strip())
                    )
                elif (
                    line.startswith("DTSTAMP:")
                    or line.startswith("DTSTART;TZID=Europe/Berlin:")
                    or line.startswith("DTEND;TZID=Europe/Berlin:")
                ):
                    current_event.append(
                        self.to_datetime(line[line.find(":") + 1 :].strip())
                    )
                elif line.startswith("\t"):
                    current_event[-1] = current_event[-1] + self.pretty_print(
